...
main(int argc, char *argv[])
{
  ...
  exit();
    ...
    1053:	68 6f 17 00 00       	push   $0x176f
    1058:	6a 02                	push   $0x2
    ...
    138c:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    13b9:	0f b6 92 ec 17 00 00 	movzbl 0x17ec(%edx),%edx
    ...
    1439:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14a9:	ff 24 85 94 17 00 00 	jmp    *0x1794(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    15b7:	90                   	nop
          s = "(null)";
    15b8:	ba 8b 17 00 00       	mov    $0x178b,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15d1:	a1 a4 1a 00 00       	mov    0x1aa4,%eax
{
  ...
  freep = p;
    ...
    1611:	5b                   	pop    %ebx
  freep = p;
    1612:	89 15 a4 1a 00 00    	mov    %edx,0x1aa4
}
    1618:	5e                   	pop    %esi
    1619:	5f                   	pop    %edi
    161a:	5d                   	pop    %ebp
  ...
  freep = p;
    164c:	89 15 a4 1a 00 00    	mov    %edx,0x1aa4
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    166c:	8b 3d a4 1a 00 00    	mov    0x1aa4,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16b1:	8b 3d a4 1a 00 00    	mov    0x1aa4,%edi
  ...
  return freep;
    16df:	8b 15 a4 1a 00 00    	mov    0x1aa4,%edx
    ...
    170c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    170f:	89 15 a4 1a 00 00    	mov    %edx,0x1aa4
}
    ...
    171f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    1720:	c7 05 a4 1a 00 00 a8 	movl   $0x1aa8,0x1aa4
    1727:	1a 00 00 
    base.s.size = 0;
    172a:	bf a8 1a 00 00       	mov    $0x1aa8,%edi
    base.s.ptr = freep = prevp = &base;
    172f:	c7 05 a8 1a 00 00 a8 	movl   $0x1aa8,0x1aa8
    1736:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    1739:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    173b:	c7 05 ac 1a 00 00 00 	movl   $0x0,0x1aac
    1742:	00 00 00 
    if(p->s.size >= nunits){
