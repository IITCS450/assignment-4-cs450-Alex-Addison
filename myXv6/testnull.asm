...
int main(void){
  ...
  printf(1, "testnull: about to dereference NULL\n");
    1011:	68 08 17 00 00       	push   $0x1708
    1016:	6a 01                	push   $0x1
    1018:	e8 c3 03 00 00       	call   13e0 <printf>
  volatile int *p = (int*)0x0;
  volatile int x = *p;
    101d:	a1 00 00 00 00       	mov    0x0,%eax
    1022:	0f 0b                	ud2    
    ...
    133c:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    1369:	0f b6 92 8c 17 00 00 	movzbl 0x178c(%edx),%edx
    ...
    13e9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1459:	ff 24 85 34 17 00 00 	jmp    *0x1734(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    1567:	90                   	nop
          s = "(null)";
    1568:	ba 2d 17 00 00       	mov    $0x172d,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1581:	a1 34 1a 00 00       	mov    0x1a34,%eax
{
  ...
  freep = p;
    ...
    15c1:	5b                   	pop    %ebx
  freep = p;
    15c2:	89 15 34 1a 00 00    	mov    %edx,0x1a34
}
    15c8:	5e                   	pop    %esi
    15c9:	5f                   	pop    %edi
    15ca:	5d                   	pop    %ebp
  ...
  freep = p;
    15fc:	89 15 34 1a 00 00    	mov    %edx,0x1a34
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    161c:	8b 3d 34 1a 00 00    	mov    0x1a34,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1661:	8b 3d 34 1a 00 00    	mov    0x1a34,%edi
  ...
  return freep;
    168f:	8b 15 34 1a 00 00    	mov    0x1a34,%edx
    ...
    16bc:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    16bf:	89 15 34 1a 00 00    	mov    %edx,0x1a34
}
    ...
    16cf:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    16d0:	c7 05 34 1a 00 00 38 	movl   $0x1a38,0x1a34
    16d7:	1a 00 00 
    base.s.size = 0;
    16da:	bf 38 1a 00 00       	mov    $0x1a38,%edi
    base.s.ptr = freep = prevp = &base;
    16df:	c7 05 38 1a 00 00 38 	movl   $0x1a38,0x1a38
    16e6:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    16e9:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    16eb:	c7 05 3c 1a 00 00 00 	movl   $0x0,0x1a3c
    16f2:	00 00 00 
    if(p->s.size >= nunits){
